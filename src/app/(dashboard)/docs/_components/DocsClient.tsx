...
function getIsRestricted(access: AccessLevel, role?: UserRole): boolean {
...
function AccessBadge({ roles }: { roles: string[] }) {
...
function RestrictedBanner({ access }: { access: UserRole[] }) {
  return (
...
function SectionH3({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-lg font-display font-semibold text-text-primary mb-3 mt-6">
      {children}
    </h3>
  );
}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-elevated border-b border-border-subtle">
            {headers.map((h) => (
              <th key={h} className="text-left px-4 py-3 text-text-secondary font-medium">
                {h}
              </th>
            ))}
          </tr>
...
const PERM_MATRIX: [string, string, string, string, string][] = [
...
export function DocsClient() {
  ...
  const Sidebar = (
    <aside className="hidden lg:flex flex-col w-60 flex-shrink-0 sticky top-0 h-screen border-r border-border-subtle bg-base overflow-y-auto">
      <div className="px-4 py-5 border-b border-border-subtle">
        ...
        <h1 className="text-sm font-display font-semibold text-text-primary mt-1">
          Titan Journal CRM
        </h1>
      </div>
        ...
        {CHAPTERS.map((ch, idx) => {
          ...
          return (
            ...
            >
              ...
              <span className="ml-auto text-xs font-mono text-text-muted opacity-50">
    ...
    <div className="flex bg-void lg:min-h-screen lg:h-full">
      ...
      <main className="flex-1 lg:overflow-y-auto">
        ...
        <div className="border-b border-border-subtle bg-base px-6 py-8">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="mt-2 text-text-secondary max-w-xl">
              Complete guide covering all features, roles, and workflows. Use the sidebar to
        ...
        <div className="max-w-3xl mx-auto px-6 py-10 space-y-24">
          ...
          {/* ═══ Ch 01 — Roles & Permissions ══════════════════════════════ */}
          <section id="roles-permissions" ref={secRef("roles-permissions")}>
            <ChapterHeader chapterMeta={CHAPTERS[0]} number={1} role={role} />
              ...
              {[
                {
                  ...
                  desc: "Platform-level administrator with full technical access. Manages system configuration, Google integrations, backups, and OWNER accounts. Typically a developer or platform operator.",
                },
                {
                  r: "OWNER",
                  style: "border-crimson/30 bg-crimson/5",
                  badge: "bg-crimson/10 text-crimson border-crimson/20",
                  desc: "IB business owner with nearly full access. Can manage leads, knowledge base, broadcasts, analytics, and team members. Has all features except Google integrations and backups.",
                },
                {
                  r: "ADMIN",
                  style: "border-blue-500/30 bg-blue-500/5",
                  badge: "bg-blue-500/10 text-blue-400 border-blue-500/20",
                  desc: "Senior team member who can manage leads, knowledge base, commands, and broadcasts, and invite STAFF. Cannot access bot settings or system-level configuration.",
                },
                {
                  r: "STAFF",
                  style: "border-border-default bg-elevated/50",
                  badge: "bg-elevated text-text-secondary border-border-default",
                  desc: "Front-line team member with limited access. Can view leads, update lead statuses, view chat history, and manage attachments. Cannot access analytics, configuration, or broadcasts.",
                },
              ].map((item) => (
                <div key={item.r} className={`rounded-xl border ${item.style} p-4`}>
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mono font-medium border ${item.badge} mb-2`}
                  >
                    {item.r}
                  </span>
                ...
                <thead>
                  <tr className="bg-elevated border-b border-border-subtle">
                    ...
                    {["SUPERADMIN", "OWNER", "ADMIN", "STAFF"].map((h) => (
                      <th
                        key={h}
                        className="text-center px-3 py-3 text-text-secondary font-medium font-mono text-xs"
                      >
                        {h}
                      </th>
                    ))}
          ...
          {/* ═══ Ch 02 — Login & Sessions ═════════════════════════════════ */}
          <section id="login-sessions" ref={secRef("login-sessions")}>
            <ChapterHeader chapterMeta={CHAPTERS[1]} number={2} role={role} />
            ...
            <Tip>
              ...
              error, your Telegram account isn&apos;t bound yet — log in once with email and
              password to link it, and future opens will be automatic.
            </Tip>
            ...
            <p className="text-text-secondary text-sm mb-4">
              Navigate to{" "}
              <strong className="text-text-primary">Settings → Sessions</strong> to view all
              active sessions. Each entry shows the device, IP address, and last active time.
              Click <strong className="text-text-primary">Revoke</strong> on any session to
          ...
          {/* ═══ Ch 03 — Dashboard & Analytics ═══════════════════════════ */}
          <section id="dashboard-analytics" ref={secRef("dashboard-analytics")}>
            <ChapterHeader chapterMeta={CHAPTERS[2]} number={3} role={role} />
            ...
            <p className="text-text-secondary text-sm mb-4">
              ...
              <strong className="text-text-primary">Active Conversations</strong>. Click any
              widget to drill into the filtered lead list for that metric.
            </p>

            <SectionH3>The Funnel</SectionH3>
            <p className="text-text-secondary text-sm mb-3">
              Leads move through a structured pipeline. Each stage represents a meaningful
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
              {[
                ...
                <React.Fragment key={s.label}>
                  <span
                    className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-mono font-medium border ${s.color}`}
                  >
                    {s.label}
                  </span>
                  {i < arr.length - 1 && (
            ...
            <p className="text-text-secondary text-sm mb-2">
              Use the date range selector at the top of the dashboard to filter all metrics.
              Available presets:
            </p>
            <div className="flex flex-wrap gap-2 mb-4">
              {["Today", "Yesterday", "This Week", "This Month", "Last 30 Days", "Last 90 Days", "All Time"].map(
                (d) => (
                  <span
                    key={d}
                    className="px-2.5 py-1 rounded-lg bg-elevated border border-border-subtle text-xs text-text-secondary font-mono"
                  >
                    {d}
                  </span>
                )
            ...
            <p className="text-text-secondary text-sm mb-3">
              The leaderboard lists your top leads sorted by AI engagement score. Scores
              indicate how conversion-ready a lead is:
            </p>
            <div className="flex gap-3 mb-4">
              {[
                ...
                <div
                  key={s.label}
                  className={`flex-1 rounded-lg border px-3 py-2 text-center ${s.c}`}
                >
            ...
            <p className="text-text-secondary text-sm mb-4">
              Click the <strong className="text-text-primary">Export CSV</strong> button at the
              top right of the dashboard to download the currently filtered dataset as a
              comma-separated values file, ready for import into Excel or Google Sheets.
            </p>

            <Warn>
              Dashboard data updates every few minutes. For real-time lead status, check the
              Lead list directly rather than relying on dashboard counters.
            </Warn>
          ...
          {/* ═══ Ch 04 — Lead Management ══════════════════════════════════ */}
          <section id="lead-management" ref={secRef("lead-management")}>
            <ChapterHeader chapterMeta={CHAPTERS[3]} number={4} role={role} />
            ...
            <p className="text-text-secondary text-sm mb-4">
              ...
              <strong className="text-text-primary">registered leads only</strong>. Multiple
            ...
            <div className="overflow-x-auto rounded-xl border border-border-subtle mb-4">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-elevated border-b border-border-subtle">
                    <th className="text-left px-4 py-3 text-text-secondary font-medium w-44">
                      Status
                    </th>
                    <th className="text-left px-4 py-3 text-text-secondary font-medium">
                      Description
                  ...
                  {[
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mono border ${badge}`}
                        >
                          {status}
                        </span>
                      </td>
            ...
            <p className="text-text-secondary text-sm mb-4">
              Click any lead row to open the full lead profile. This displays: contact info
              (name, Telegram ID, email, phone), a complete activity timeline, deposit history,
              an AI-generated conversation summary, and a detailed AI score breakdown showing
              exactly why the score was assigned.
            </p>
            ...
            <p className="text-text-secondary text-sm mb-4">
              Toggle the <strong className="text-text-primary">Handover</strong> switch on a
              lead to pause the bot and allow your team to reply manually. The bot will not send
              any automated responses while handover is active. Toggle it off to re-enable the
              bot for that lead.
            </p>

            <SectionH3>Bulk Actions</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Select multiple leads using the checkboxes on the left of each row. A bulk action
              toolbar appears at the bottom with:{" "}
              <strong className="text-text-primary">Bulk Update Status</strong> and{" "}
              <strong className="text-text-primary">Bulk Toggle Handover</strong>. Ideal for
              batch-processing large segments of your pipeline.
            </p>

            <Tip>
              STAFF users can view leads and update statuses but cannot access analytics,
              configuration, or broadcasts. Contact your OWNER or ADMIN for a role upgrade if
              needed.
            </Tip>
          ...
          {/* ═══ Ch 05 — Interactions & Chat ══════════════════════════════ */}
          <section id="interactions-chat" ref={secRef("interactions-chat")}>
            <ChapterHeader chapterMeta={CHAPTERS[4]} number={5} role={role} />

            <SectionH3>Viewing Conversations</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Open any lead profile and navigate to the{" "}
              <strong className="text-text-primary">Interactions</strong> tab. You will see a
              chronological timeline of all messages, bot replies, manual replies, and system
              events for that lead.
            </p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {[
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <code className="text-xs font-mono text-crimson">{type}</code>
                      </td>
                      <td className="px-4 py-2.5 text-text-secondary">{desc}</td>
                    </tr>
                  ))}
            ...
            />
            <Tip>
              Always enable Handover Mode before sending manual replies. Without it, the bot
              may also respond to the lead simultaneously, causing duplicate messages.
            </Tip>
          ...
          {/* ═══ Ch 06 — Attachments & Media ══════════════════════════════ */}
          <section id="attachments-media" ref={secRef("attachments-media")}>
            <ChapterHeader chapterMeta={CHAPTERS[5]} number={6} role={role} />

            <SectionH3>How Leads Submit Proof</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Leads submit deposit proof directly through the Telegram bot by sending a
              screenshot or video to the chat. The CRM automatically captures and stores the
              file, linking it to the lead&apos;s profile. No manual upload is required from
              the team.
            </p>
            ...
            <p className="text-text-secondary text-sm mb-4">
              Open any lead profile and navigate to the{" "}
              <strong className="text-text-primary">Attachments</strong> tab. Files are
              displayed in a thumbnail grid. Click any thumbnail to open the full-size viewer.
            </p>
            ...
            />
            <Warn>
              Once verified, the deposit confirmation is recorded in the Audit Log and cannot
              be undone automatically. To reverse, manually update the lead status and note the
              correction.
            </Warn>
          ...
          {/* ═══ Ch 07 — Knowledge Base ═══════════════════════════════════ */}
          <section id="knowledge-base" ref={secRef("knowledge-base")}>
            <ChapterHeader chapterMeta={CHAPTERS[6]} number={7} role={role} />

            <SectionH3>What the Knowledge Base Does</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Every active Knowledge Base entry is embedded as a vector and retrieved by the AI
              when a lead asks a question. The AI searches for relevant entries and uses them to
              construct accurate, on-brand answers. A richer Knowledge Base means better, more
              accurate bot responses.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              {[
                ...
                <div
                  key={item.type}
                  className="rounded-xl border border-border-default bg-elevated/50 p-4"
                >
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {[
                ...
                <span
                  key={s}
                  className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-mono border ${c}`}
                >
                  {s}
                </span>
              ))}
            ...
            <p className="text-text-secondary text-sm mb-4">
              ...
              <code className="font-mono text-xs text-success">ACTIVE</code>. Failed entries
              must be deleted and re-created.
            </p>

            <SectionH3>Enable / Disable Entries</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Toggle the switch on any KB entry to include or exclude it from AI retrieval
              without permanently deleting it. Disabled entries are ignored by the AI until
              re-enabled.
            </p>
            <Tip>
              Disable entries during testing or review so they don&apos;t affect live bot
              responses while you refine the content.
            </Tip>
          ...
          {/* ═══ Ch 08 — Command Menu ═════════════════════════════════════ */}
          <section id="command-menu" ref={secRef("command-menu")}>
            <ChapterHeader chapterMeta={CHAPTERS[7]} number={8} role={role} />

            <SectionH3>What Commands Do</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Telegram bot commands are shortcuts that begin with{" "}
              <code className="font-mono text-xs bg-elevated px-1 py-0.5 rounded text-crimson">
                /
              </code>
              . When a lead types a command (e.g.{" "}
              <code className="font-mono text-xs bg-elevated px-1 py-0.5 rounded">
                /register
              </code>
              ), the bot immediately sends the pre-configured reply — no AI processing required.
              Use commands for frequently requested links, instructions, or information.
            </p>

            <SectionH3>Viewing Commands</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Navigate to{" "}
              <strong className="text-text-primary">Settings → Commands</strong> to see the
              full list of configured commands, their descriptions, and their reply content.
            </p>
            ...
            <p className="text-text-secondary text-sm mb-4">
              Click the <strong className="text-text-primary">edit icon</strong> next to any
              command to open the edit form. Update any fields and click Save. Changes sync to
              Telegram immediately.
            </p>

            <SectionH3>Deleting a Command</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Click the <strong className="text-text-primary">delete icon</strong> on a command
              and confirm the deletion. The command is removed from the Telegram bot menu
              immediately.
            </p>
          ...
          {/* ═══ Ch 09 — Broadcast Messaging ══════════════════════════════ */}
          <section id="broadcast-messaging" ref={secRef("broadcast-messaging")}>
            <ChapterHeader chapterMeta={CHAPTERS[8]} number={9} role={role} />

            <SectionH3>What a Broadcast Is</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              A broadcast sends one message to all leads simultaneously — every lead with a
              connected Telegram account will receive it. Use broadcasts for announcements,
              promotional campaigns, platform updates, or re-engagement campaigns.
            </p>
            ...
            <p className="text-text-secondary text-sm mb-4">
              View all past broadcasts in the Broadcasts page. Each entry shows: message
              preview, status (
              ...
              <code className="font-mono text-xs text-danger">FAILED</code>), recipient count,
              and timestamp.
            </p>

            <Warn>
              Broadcasts send to ALL connected leads simultaneously. There is no way to undo or
              recall a broadcast once confirmed. Always review your message carefully before
              confirming.
            </Warn>
            <Warn>
              If the broadcast button is disabled or you receive a 403 error, the Broadcast
              feature has been turned off in System Configuration by your SUPERADMIN. Contact
          ...
          {/* ═══ Ch 10 — Users & Invitations ══════════════════════════════ */}
          <section id="users-invitations" ref={secRef("users-invitations")}>
            <ChapterHeader chapterMeta={CHAPTERS[9]} number={10} role={role} />

            <SectionH3>Viewing Users</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Navigate to{" "}
              <strong className="text-text-primary">Settings → Team</strong> to see all users
              with their role, status (active / inactive), and last login timestamp.
            </p>
            ...
            <p className="text-text-secondary text-sm mb-4">
              If the invitee hasn&apos;t accepted yet, click{" "}
              <strong className="text-text-primary">Resend</strong> next to their pending invite
              in the Team tab. A new invitation email will be dispatched.
            </p>

            <SectionH3>Revoking an Invitation</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Cancel a pending invite by clicking{" "}
              <strong className="text-text-primary">Revoke</strong> next to it. This invalidates
              the setup link before the invitee can use it.
            </p>

            <SectionH3>Deactivating & Reactivating a User</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Click the menu (
              <strong className="text-text-primary">⋯</strong>) on any active user and select{" "}
              <strong className="text-text-primary">Deactivate</strong>. The user immediately
              loses access to the CRM. To restore access, click{" "}
              <strong className="text-text-primary">Reactivate</strong> on a deactivated user.
            </p>

            <SectionH3>Role Limits</SectionH3>
            <SimpleTable
              headers={["Your Role", "Can Invite / Manage"]}
              rows={[
                ["SUPERADMIN", "OWNER, ADMIN, STAFF"],
                ["OWNER", "ADMIN, STAFF"],
                ["ADMIN", "STAFF only"],
          ...
          {/* ═══ Ch 11 — Audit Logs ═══════════════════════════════════════ */}
          <section id="audit-logs" ref={secRef("audit-logs")}>
            <ChapterHeader chapterMeta={CHAPTERS[10]} number={11} role={role} />
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {[
                ...
                <code
                  key={a}
                  className="text-xs font-mono px-2 py-1 rounded bg-elevated border border-border-subtle text-text-secondary"
                >
                  {a}
                </code>
              ))}
            ...
            <p className="text-text-secondary text-sm mb-4">
              Navigate to <strong className="text-text-primary">Audit Logs</strong> in the
              sidebar. Logs are displayed in reverse-chronological order. Each entry shows: the
              actor (who did it), the action type, the affected resource, and the exact
              timestamp.
            </p>
            ...
            <p className="text-text-secondary text-sm mb-4">
              ...
              <strong className="text-text-primary">date range</strong>. Combine multiple
              filters to find specific events quickly.
            </p>

            <SectionH3>Before / After Values</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Expand any log entry by clicking on it to see the full before-and-after diff. For
              example, a{" "}
              <code className="font-mono text-xs text-crimson">LEAD_STATUS_CHANGED</code> entry
              shows the previous status and the new status side by side.
            </p>
            ...
            <div className="rounded-lg border border-border-subtle bg-elevated/50 px-4 py-3 text-sm text-text-secondary mb-4">
              By design,{" "}
              <strong className="text-text-primary">OWNER</strong> and{" "}
              <strong className="text-text-primary">ADMIN</strong> users cannot see audit log
              entries generated by{" "}
              <strong className="text-text-primary">SUPERADMIN</strong> users. This is
              intentional to separate platform-level operations from business-level activity.
            </div>
          ...
          {/* ═══ Ch 12 — System Configuration ════════════════════════════ */}
          <section id="system-configuration" ref={secRef("system-configuration")}>
            <ChapterHeader chapterMeta={CHAPTERS[11]} number={12} role={role} />

            <SectionH3>Feature Flags</SectionH3>
            <p className="text-text-secondary text-sm mb-3">
              Toggle switches in System Configuration control which features are available
            ...
            <div className="space-y-2 mb-4">
              {[
                ...
                <div
                  key={f.name}
                  className="flex items-start gap-3 rounded-lg border border-border-subtle bg-elevated/40 px-4 py-3"
                >
            ...
            <p className="text-text-secondary text-sm mb-4">
              Enable Maintenance Mode to pause the bot and display a maintenance banner across
              the dashboard for all users. Set a custom maintenance message to inform your team
              of what&apos;s happening and the expected resolution time.
            </p>
            ...
            <div className="space-y-2 mb-4">
              {[
                ...
                <div key={field} className="flex items-start gap-3">
                  <code className="text-xs font-mono text-crimson bg-crimson/5 border border-crimson/20 px-2 py-1 rounded whitespace-nowrap mt-0.5">
                    {field}
                  </code>
            ...
            <div className="space-y-2 mb-4">
              {[
                ...
                <div key={field} className="flex items-start gap-3">
                  <code className="text-xs font-mono text-blue-400 bg-blue-500/5 border border-blue-500/20 px-2 py-1 rounded whitespace-nowrap mt-0.5">
                    {field}
                  </code>
                  <p className="text-sm text-text-secondary">{desc}</p>
                </div>
            ...
            <p className="text-text-secondary text-sm mb-4">
              ...
              <strong className="text-text-primary">Role Title</strong> (e.g. &ldquo;Account
              Manager&rdquo;), and{" "}
              <strong className="text-text-primary">Brand Name</strong>. These values are
              injected into the system prompt to give the bot a consistent identity.
            </p>

            <Warn>
              Changes to the System Prompt or AI settings take effect immediately and change bot
              behaviour for all new conversations. Test changes carefully before saving in
              production.
            </Warn>
          ...
          {/* ═══ Ch 13 — Bot Settings ═════════════════════════════════════ */}
          <section id="bot-settings" ref={secRef("bot-settings")}>
            <ChapterHeader chapterMeta={CHAPTERS[12]} number={13} role={role} />

            <SectionH3>Bot Active Toggle</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              The <strong className="text-text-primary">Bot Active</strong> toggle completely
              enables or disables the Telegram bot. When turned off, all incoming messages from
              leads are silently ignored — no replies are sent and no interactions are logged.
              Use this during maintenance or emergencies.
            </p>

            <SectionH3>Reply Templates</SectionH3>
            <p className="text-text-secondary text-sm mb-3">
              Configure canned responses for these system events:
            </p>
            <div className="space-y-2 mb-4">
              {[
            ...
            <p className="text-text-secondary text-sm mb-4">
              ...
              <strong className="text-text-primary">max messages per window</strong>. Leads who
              send more messages than the limit within the window will receive the rate limit
              reply and no AI response until the window resets.
            </p>

            <SectionH3>Suspicion Detection</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Configure a <strong className="text-text-primary">max strikes</strong> count and a{" "}
              <strong className="text-text-primary">strike window</strong>. If a lead sends
              messages detected as suspicious or injection attempts within the window, they are
              flagged and receive the injection detection reply.
            </p>

            <SectionH3>HyDE — Hypothetical Document Embedding</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              HyDE is an AI technique where the model first generates a hypothetical ideal
              answer, then uses that as the search query against the Knowledge Base. This
              significantly improves retrieval quality for abstract or indirect questions.{" "}
              <strong className="text-text-primary">Enable</strong> for better KB results;{" "}
              <strong className="text-text-primary">disable</strong> for faster but less nuanced
              retrieval.
            </p>

            <SectionH3>Admin Telegram Commands</SectionH3>
            <p className="text-text-secondary text-sm mb-3">
              Admin users can control the CRM directly from their personal Telegram chat with
              the bot:
            </p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {[
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <code className="text-xs font-mono text-crimson">{cmd}</code>
                      </td>
                      <td className="px-4 py-2.5 text-text-secondary">{desc}</td>
                    </tr>
                  ))}
          ...
          {/* ═══ Ch 14 — Google & Backup ══════════════════════════════════ */}
          <section id="google-backup" ref={secRef("google-backup")}>
            <ChapterHeader chapterMeta={CHAPTERS[13]} number={14} role={role} />
            ...
            <Tip>
              ...
              (every day at 2:00 AM) with a 30-day retention period to balance safety and
              storage cost.
            </Tip>

            <SectionH3>Triggering a Manual Backup</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              Click <strong className="text-text-primary">Backup Now</strong> to trigger an
              immediate backup. A confirmation dialog will show the destination and estimated
              size. Click Confirm to start the backup job in the background.
            </p>

            <SectionH3>Backup History</SectionH3>
            <p className="text-text-secondary text-sm mb-4">
              The backup history table shows each backup with: date and time, file size,
              destination (S3 or Google Drive), and status (
              <code className="font-mono text-xs text-success">COMPLETED</code> or{" "}
              <code className="font-mono text-xs text-danger">FAILED</code>).
            </p>

            <Warn>
              Restoring a backup requires server-level access and cannot be performed from the
              dashboard. Contact your system administrator to initiate a restore procedure.
            </Warn>
          ...
          {/* ═══ Ch 15 — Troubleshooting & FAQ ═══════════════════════════ */}
          <section id="troubleshooting" ref={secRef("troubleshooting")}>
            <ChapterHeader chapterMeta={CHAPTERS[14]} number={15} role={role} />

            <div className="space-y-4">
              {[
                {
                  q: "Bot is not responding to leads",
                  a: "Check the Bot Active toggle in Bot Settings — it may have been turned off. Also verify that Maintenance Mode is not enabled in System Configuration. If the bot is active but still unresponsive, check the rate limit settings and confirm the Telegram webhook is connected.",
                },
                {
                  q: '"Broadcast feature is currently disabled" error',
                  a: "This means the Broadcast feature flag has been disabled in System Configuration. Contact your SUPERADMIN and ask them to enable the feature.broadcast.enabled setting.",
                },
                {
                  q: '"Insufficient permissions" error',
                  a: "Your current role does not have access to the feature you are trying to use. Refer to the Permissions Matrix in Chapter 1 to understand what your role can access. Contact your OWNER or ADMIN to request a role upgrade if needed.",
                },
                {
                  q: "Lead status is not updating",
                  a: "Confirm you are logged in as OWNER, ADMIN, or STAFF with the correct permissions. If the problem persists, try refreshing the page — status updates require an active internet connection.",
                },
                {
                  q: "Invitation email was not received",
                  a: "Ask the invitee to check their spam or junk folder first. If the email is not there, an OWNER or ADMIN can resend the invitation from Settings → Team by clicking Resend next to the pending invite.",
                },
                {
                  q: "Session expired and I cannot log in",
                  a: "Clear your browser cookies and cache, then navigate to the login page and sign in fresh. For the Telegram Mini App, close and re-open the app from Telegram. If you are still unable to log in, contact your OWNER or ADMIN.",
                },
                {
                  q: 'Knowledge Base entry is stuck on "PENDING"',
                  a: "Processing typically takes 1–2 minutes for most content and up to 5 minutes for large files. If an entry has been PENDING for more than 5 minutes, delete it and re-upload. If the issue persists, check with your SUPERADMIN that the embedding service is running.",
                },
                {
                  q: "Dashboard analytics showing zero for all metrics",
                  a: "Check the date range filter — it may be set to a period with no data. Try switching to All Time. Dashboard data is updated every few minutes, so very recent leads may not appear immediately.",
                },
                {
                  q: "Can I undo a status change?",
                  a: "Yes. Open the lead profile and change the status back to the previous value manually. All status changes are recorded in the Audit Logs, so you always have a complete history of what was changed and when.",
                },
                {
                  q: "Where do I find my bot's Telegram username?",
                  a: "The Telegram bot username was configured during initial platform setup by your SUPERADMIN. Check with them or look in Bot Settings — the username is tied to the configured bot token.",
                },
              ].map(({ q, a }, i) => (
                <div
                  key={i}
                  className="rounded-xl border border-border-subtle bg-elevated/30 p-5"
                >
                  ...
                  <div className="flex gap-3 items-start mt-3">
                    <span className="flex-shrink-0 w-6 h-6 rounded-full bg-success/10 text-success text-xs font-mono font-bold flex items-center justify-center mt-0.5">
                      A
                    </span>
