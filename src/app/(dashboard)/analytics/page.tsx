...
function formatXLabel(dateStr: string, timeframe: Timeframe): string {
  switch (timeframe) {
    ...
    default: {
      ...
      const d = new Date((dateStr.slice(0, 7) || dateStr.slice(0, 10)) + "-15T12:00:00");
      return isNaN(d.getTime())
        ? dateStr.slice(0, 7)
...
// ── Tooltip components ───────────────────────────────────────────
function AreaTip({ active, payload, label }: { active?: boolean; payload?: any[]; label?: string }) {
  if (!active || !payload?.length) return null;
  return (
    <div style={{ background: "#0e0e1a", border: "1px solid #2a2a42", borderRadius: 12, padding: "10px 14px", boxShadow: "0 8px 32px rgba(0,0,0,0.7)" }}>
      <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 8, fontFamily: "inherit" }}>{label}</p>
      {payload.map((e: any) => (
        <div key={e.name} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: e.stroke ?? e.fill, flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", fontFamily: "inherit" }}>{e.name}</span>
          <span style={{ fontSize: 12, color: "#fff", fontFamily: "var(--font-jetbrains-mono, monospace)", marginLeft: "auto", paddingLeft: 12 }}>
...
function BarTip({ active, payload, label }: { active?: boolean; payload?: any[]; label?: string }) {
  if (!active || !payload?.length) return null;
  return (
    <div style={{ background: "#0e0e1a", border: "1px solid #2a2a42", borderRadius: 12, padding: "10px 14px" }}>
      <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>{label}</p>
      <p style={{ fontSize: 14, color: "#E8B94F", fontFamily: "var(--font-jetbrains-mono, monospace)" }}>
        {payload[0].value} <span style={{ fontSize: 11, color: "rgba(255,255,255,0.3)" }}>leads</span>
      </p>
    </div>
  );
}
...
function ChartInView({ children, className }: { children: React.ReactNode; className?: string }) {
...
function StatChipBadge({
  ...
  return (
    <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full surface-card border border-border-subtle">
      <span
...
function KpiCard({
  ...
  return (
    <div className="kpi-stat-card kpi-card p-5">
      <div className="flex items-start justify-between mb-4">
        ...
        >
          {up ? <ArrowUpRight size={12} weight="regular" /> : <ArrowDownRight size={12} weight="regular" />}
          {change}
        </span>
      </div>
      <p className="font-display font-extrabold text-[28px] text-text-primary leading-none mb-1.5 tracking-tight">
        {value}
      </p>
...
export default function AnalyticsPage() {
  ...
  const funnelData = useMemo(() => {
    ...
    return [
      ...
      { name: "Registered",   value: summary.funnel.registered,       fill: FUNNEL_FILLS.registered },
      { name: "Dep. Reported",value: summary.funnel.depositReported,  fill: FUNNEL_FILLS.depositReported },
      { name: "Confirmed",    value: summary.funnel.depositConfirmed, fill: FUNNEL_FILLS.depositConfirmed },
    ].filter((f) => f.value > 0);
  }, [summary]);
  ...
  useGSAP(() => {
    gsap.from(".kpi-stat-card", {
      opacity: 0, y: 18, stagger: 0.08, duration: 0.45, ease: "power2.out",
    });
    gsap.from(".funnel-bar", {
      width: 0, duration: 0.6, ease: "power2.out", stagger: 0.07,
    });
  ...
  const handleTimeframeChange = (tf: Timeframe) => {
    ...
    setTimeout(() => { setTimeframe(tf); setContentVisible(true); }, 250);
  };
  ...
  return (
    <div ref={containerRef} className="space-y-5 animate-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-4 justify-between">
        ...
        <div
          className="surface-card p-1 flex items-center gap-0.5 rounded-xl self-start sm:self-auto shrink-0 flex-wrap"
          role="tablist"
        ...
        >
          {TIMEFRAMES.map((tf) => (
            <button
              ...
              className={"px-3.5 py-1.5 rounded-lg text-xs font-sans font-medium transition-all cursor-pointer " + (timeframe === tf ? "bg-crimson text-white" : "text-text-secondary hover:text-text-primary")}
            >
              {TIMEFRAME_LABELS[tf]}
            </button>
          ))}
      ...
      >
          ...
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
            ...
            <ChartInView className="xl:col-span-3 chart-card p-5">
              ...
              <p className="text-xs font-sans mb-5" style={{ color: "rgba(255,255,255,0.35)" }}>
              ...
              <ResponsiveContainer width="100%" height={168}>
                <AreaChart data={trendData} margin={{ top: 0, right: 0, left: -22, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="New Leads" stroke="#60a5fa" strokeWidth={2} fill="url(#gNew)" />
                  <Area type="monotone" dataKey="Registered" stroke="#a855f7" strokeWidth={2} fill="url(#gReg)" />
                  <Area type="monotone" dataKey="Confirmed" stroke="#22d3a0" strokeWidth={2} fill="url(#gDep)" />
                </AreaChart>
              </ResponsiveContainer>
              <div
              ...
              >
                {([["#60a5fa", "New Leads"], ["#a855f7", "Registered"], ["#22d3a0", "Confirmed"]] as const).map(([color, label]) => (
            ...
            {/* Conversion funnel */}
            <ChartInView className="xl:col-span-2 surface-card p-5">
              <div className="flex items-center justify-between mb-1">
                <h2 className="font-sans font-semibold text-[15px] text-text-primary">
                  {t("analytics.charts.funnelBreakdown")}
                </h2>
                <div className="ios-icon ios-icon-sm" style={{ background: "rgba(196,35,45,0.12)", color: "#C4232D" }}>
                    ...
                    <div key={step.name}>
                      {dropPct !== null && (
                        <div className="flex items-center justify-center py-0.5">
                          <span className="text-[10px] font-sans text-text-muted flex items-center gap-0.5">
                            <ArrowsLeftRight size={10} weight="regular" className="rotate-90" />
                            {dropPct}% drop-off
                          </span>
                        </div>
                      )}
                      <div className="flex items-center gap-3">
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[12px] font-sans text-text-secondary">{step.name}</span>
                            <span className="data-mono text-[12px]">{step.value.toLocaleString()}</span>
                          </div>
                        ...
                        <span className="text-[11px] font-mono w-9 text-right" style={{ color: step.fill }}>
                          {pct}%
                        </span>
                      </div>
                    </div>
              ...
              {summary?.funnel.conversionRates && (
                <div className="mt-4 pt-3 border-t border-border-subtle space-y-1">
                  {[
                    ["New → Registered",  summary.funnel.conversionRates.newToRegistered],
                    ["Reg → Reported",    summary.funnel.conversionRates.registeredToReported],
                    ["Reported → Confirmed", summary.funnel.conversionRates.reportedToConfirmed],
                  ].map(([label, val]) => (
                    <div key={label as string} className="flex justify-between">
                      <span className="text-[10px] text-text-muted">{label}</span>
                      <span className="text-[10px] font-mono text-text-secondary">{(val as number).toFixed(1)}%</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            <ChartInView className="chart-card p-5">
              <div className="flex items-center justify-between mb-1">
                <h2 className="font-sans font-semibold text-[15px] text-white">
                  {t("analytics.charts.leadSources")}
                </h2>
                <div className="ios-icon ios-icon-sm" style={{ background: "rgba(232,185,79,0.12)", color: "#E8B94F" }}>
                ...
                >
                  <defs>
                    <linearGradient id="gSrc" x1="0" y1="0" x2="1" y2="0">
                      ...
                      <stop offset="100%" stopColor="#C4232D" stopOpacity={0.35} />
                    </linearGradient>
                  </defs>
                  ...
                  <YAxis
                    ...
                    type="category"
                    width={130}
                    tick={{ fontSize: 11, fill: "rgba(255,255,255,0.45)", fontFamily: "inherit" }}
                    axisLine={false}
                    tickLine={false}
                  />
              ...
              <div className="mt-4 pt-3.5 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                  {funnelData.map((s) => {
                    const pct = funnelData[0]?.value > 0 ? Math.round((s.value / funnelData[0].value) * 100) : 0;
                    return (
                      <div key={s.name} className="flex items-center justify-between">
                        <span className="text-[11px] font-sans" style={{ color: "rgba(255,255,255,0.45)" }}>
                          {s.name}
                        </span>
            ...
            <ChartInView className="chart-card p-5">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="ios-icon ios-icon-sm" style={{ background: "rgba(168,85,247,0.12)", color: "#a855f7" }}>
              ...
              <ResponsiveContainer width="100%" height={120}>
                <BarChart data={funnelData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }} barSize={32}>
                  <defs>
                    {funnelData.map((f, i) => (
                      <linearGradient key={i} id={`fGrad${i}`} x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%"   stopColor={f.fill} stopOpacity={0.85} />
                        <stop offset="100%" stopColor={f.fill} stopOpacity={0.4}  />
                      </linearGradient>
                    ))}
                  </defs>
                  <XAxis
                    dataKey="name"
                    tick={{ fontSize: 10, fill: "rgba(255,255,255,0.35)", fontFamily: "inherit" }}
                    axisLine={false}
                    tickLine={false}
                  />
                  <YAxis hide />
                  <Tooltip
                    content={({ active, payload, label }: any) =>
                      active && payload?.length ? (
                        <div style={{ background: "#0e0e1a", border: "1px solid #2a2a42", borderRadius: 10, padding: "8px 12px" }}>
                          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>{label}</p>
                          <p style={{ fontSize: 14, color: payload[0].fill, fontFamily: "var(--font-jetbrains-mono, monospace)" }}>
